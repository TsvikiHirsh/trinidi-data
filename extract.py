...
def get_file_name(isotope, z_numbers, data_dir):
    ...
    Z = z_numbers[symbol]
    ZA = Z * 1000 + A;
    basename = str(ZA) + ".800nc"
    file_name = data_dir + "/" + symbol + "/" + basename
    return file_name
...
for i, iso in enumerate(isotopes):
    ...
    print(f'   {i+1}/{len(isotopes)}: {iso}')

    file_name = get_file_name(iso, z_numbers, data_dir)
    if(not os.path.exists(file_name)):
        raise Exception(f'file_name: `{file_name}` does not exist')

    e, x = read_file(file_name)
